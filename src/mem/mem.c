...
uint8_t mem_load_program(const char* path, uint16_t base_addr) {
    if (base_addr < MEM_PROGRAM_ROM_START || base_addr > MEM_PROGRAM_ROM_END) {
        fprintf(stderr, "[FAILED] Program base must be in $8000-$BFFF.\n");
        return 1;
    }
...
void mem_set_reset_vector(uint16_t addr) {
    ...
    mem_force_write(MEM_VECTOR_RESET, addr & 0x00FF);
    mem_force_write(MEM_VECTOR_RESET + 1, (addr >> 8) & 0x00FF);
}
...
void mem_init(char* filename) {
    ...
    if (mem_load_program(filename, MEM_PROGRAM_ROM_START) != 0) {
        exit(1);
    }
    mem_set_reset_vector(MEM_PROGRAM_ROM_START);
}
